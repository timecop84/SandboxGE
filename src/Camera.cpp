...
Camera::Camera(const Vector &from, const Vector &to, const Vector &up, int type)
    : m_eye(from.m_x, from.m_y, from.m_z), m_look(to.m_x, to.m_y, to.m_z),
      m_up(up.m_x, up.m_y, up.m_z) {
  m_projectionType = (type == 0) ? PERSPECTIVE : ORTHOGRAPHIC;

  // Initialize default perspective parameters (CRITICAL FIX!)
  m_fovy = 45.0f;
  m_aspect = 1.0f;
  m_znear = 0.1f;
  m_zfar = 1000.0f;

  // Initialize default orthographic parameters
  m_left = -1.0f;
  m_right = 1.0f;
  m_bottom = -1.0f;
  m_top = 1.0f;
...
void Camera::set(const glm::vec3 &from, const glm::vec3 &to,
                 const glm::vec3 &up) {
  m_eye = from;
  m_look = to;
  m_up = up;
...
void Camera::setShape(float fov, float aspect, float near, float far,
                      int projType) {
  if (projType == PERSPECTIVE) {  // Use enum value
...
void Camera::updateProjectionMatrix() {
  if (m_projectionType == PERSPECTIVE) {
    m_projectionMatrix =
        glm::perspective(glm::radians(m_fovy), m_aspect, m_znear, m_zfar);
  } else {
    m_projectionMatrix =
